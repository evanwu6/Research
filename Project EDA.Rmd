...
arsenal_spin <- read_csv("App Comp Data/arsenal_2022.csv")
...
pitchers %>%
  mutate(ID_num = case_when(ID == "Great" ~ 1,
                            ID == "Decent" ~ 2,
                            ID == "Bad" ~ 3)) %>% 
  filter(!(pitch_name %in% c(NA, "Pitch Out"))) %>%
  mutate(facet_label = paste0(player_name, " (", ID, ")"),
         facet_label = fct_reorder(facet_label, ID_num, .desc = FALSE)) %>%
  ggplot(aes(x = -pfx_x * 12, y = pfx_z * 12)) +
  ...
  labs(
    title = "Pitch Movement of Pitch Arsenal",
    subtitle = "2022 Season",
...
# Distance vs. RE
lhp %>% 
  ggplot(aes(x = distance, y = run_exp_added, color = pitch_name)) +
