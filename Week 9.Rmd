...
fb_r_whiff_test <- augment(fb_r_whiff)
  
fb_r_whiff_test <- fb_r_whiff_test %>% 
  mutate(prediction_log = predict(fb_r_whiff, fb_r_whiff_test),
...
# Refined Model
fb_r_barrel <- glm(Barrel ~ pitch_speed + dist_x + dist_z +
                  I(dist_x^2) + I(dist_z^2) + release_spin_rate + pred_bwhiff,
              data = fb_r, family = binomial)

# Removed: pfx_z, dist_prop, pfx_x, pfx_total, zone (multiple at 0.05-0.06)

# Model Evaluation
# summary(fb_r_barrel)

fb_r_barrel_test <- augment(fb_r_barrel)
  
fb_r_barrel_test <- fb_r_barrel_test %>% 
  mutate(prediction_log = predict(fb_r_barrel, fb_r_barrel_test),
         prediction = 1 / (1 + exp(-prediction_log)),
         rounded_pred = case_when(
           prediction >= 0.5 ~ 1,
           prediction < 0.5 ~ 0
...
fb_r_strike_test <- augment(fb_r_strike)
  
fb_r_strike_test <- fb_r_strike_test %>% 
  mutate(prediction_log = predict(fb_r_strike, fb_r_strike_test),
         prediction = 1 / (1 + exp(-prediction_log)),
         rounded_pred = case_when(
           prediction >= 0.5 ~ 1,
           prediction < 0.5 ~ 0
...
fb_l_whiff_test <- augment(fb_l_whiff)
  
fb_l_whiff_test <- fb_l_whiff_test %>% 
  mutate(prediction_log = predict(fb_l_whiff, fb_l_whiff_test),
         prediction = 1 / (1 + exp(-prediction_log)),
         rounded_pred = case_when(
           prediction >= 0.5 ~ 1,
           prediction < 0.5 ~ 0
...
fb_l_barrel_test <- augment(fb_l_barrel)
  
fb_l_barrel_test <- fb_l_barrel_test %>% 
  mutate(prediction_log = predict(fb_l_barrel, fb_l_barrel_test),
         prediction = 1 / (1 + exp(-prediction_log)),
         rounded_pred = case_when(
           prediction >= 0.5 ~ 1,
           prediction < 0.5 ~ 0
...
fb_l_strike_test <- augment(fb_l_strike)
  
fb_l_strike_test <- fb_l_strike_test %>% 
  mutate(prediction_log = predict(fb_l_strike, fb_l_strike_test),
         prediction = 1 / (1 + exp(-prediction_log)),
         rounded_pred = case_when(
           prediction >= 0.5 ~ 1,
           prediction < 0.5 ~ 0
