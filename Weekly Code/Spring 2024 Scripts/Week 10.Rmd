...
# Refined Model
fb_r_strike <- glm(STRIKE ~ pfx_z + zone + dist_x + dist_z + dist_prop + 
...
```{r CU RHP RHH, warning = FALSE, message = FALSE}
cu_r <- rhp %>% 
...
```{r CU RHP LHH, warning = FALSE, message = FALSE}
cu_l <- rhp %>% 
  filter(pitch_type == "CU",
         hitter == "L") %>% 
  mutate(Barrel = ifelse(Barrel == TRUE, 1, 0),
...
```{r CH RHP RHH, warning = FALSE, message = FALSE}
ch_r <- rhp %>% 
...
```{r CH RHP LHH, warning = FALSE, message = FALSE}
ch_l <- rhp %>% 
  filter(pitch_type == "CH",
         hitter == "L") %>% 
  mutate(Barrel = ifelse(Barrel == TRUE, 1, 0),
